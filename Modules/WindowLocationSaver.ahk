...
global CrLf := "`r`n"
global FileName := ".\Refs\WinPos.txt"

WindowLocationSaverDef(){
	Hotkey, #0, RestoreWindows
	Hotkey, #+0, SaveWindowsToFile
	Return "Win-0: Restore Windows`n Win-Shift-0: Save Windows`n"
}
...
RestoreWindows(){
  ...
  if !SectionFound
  {
    msgbox,,Dock Windows, Section does not exist in %FileName% `nLooking for: %SectionToFind%`n`nTo save a new section, use Win-Shift-0 (zero key above letter P on leopard)
  }
